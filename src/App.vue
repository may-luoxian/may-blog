    ...
    const fetchWebsiteConfig = async () => {
      try {
        ...
        if (appConfig.customStyle) {
          let { websiteStyle } = data.data.websiteConfigDTO
          appStore.themeConfig.gradient.color_1 = websiteStyle.leftTopColor
          appStore.themeConfig.gradient.color_2 = websiteStyle.centerColor
          appStore.themeConfig.gradient.color_3 = websiteStyle.rightBottomColor
          appStore.themeConfig.header_gradient_css =
            `linear-gradient(130deg, ${websiteStyle.leftTopColor}, ${websiteStyle.centerColor} 41.07%, ${websiteStyle.rightBottomColor} 76.05%)`
          appStore.themeConfig.background_gradient_style.background =
            `linear-gradient(130deg, ${websiteStyle.leftTopColor}, ${websiteStyle.centerColor} 41.07%, ${websiteStyle.rightBottomColor} 76.05%)`
          let root = document.documentElement
          root.style.setProperty('--main-gradient', `linear-gradient(130deg, ${websiteStyle.leftTopColor}, ${websiteStyle.centerColor} 41.07%, ${websiteStyle.rightBottomColor} 76.05%)`)
        }
      } catch (err) {
        console.error(err)
      }
